...
async function main() {
  ...
  console.log("── v0.6: dev toolkit ──");
  const pwOut = await run("pw 24 --no-symbols");
  ok("pw: exact length", (pwOut[0] ?? "").length === 24);
  ok("pw: only unambiguous alphanumerics", /^[A-HJ-NP-Za-km-z2-9]+$/.test(pwOut[0] ?? ""));
  ok("pw: telemetry line", pwOut.some((l) => l.includes("crypto.getRandomValues")));
  ok("pw: min length clamp", (await run("pw 2"))[0]?.length === 8);
  ok("pw: count flag", (await run("pw 12 --count 3")).filter((l) => l.length === 12).length === 3);
  ok("base: dec→hex", (await run("base 10:16 255")).join("").includes("0xff"));
  ok("base: hex→dec", (await run("base 16:10 0xff")).join("").includes("255"));
  ok("base: bin→dec", (await run("base 2:10 1010")).join("").includes("10"));
  ok("base: invalid digit refused", (await run("base 2:10 10201")).some((l) => l.includes("not a valid base-2")));
  ok("base: bad bases refused", (await run("base 7:10 42")).some((l) => l.includes("2, 8, 10, 16")));
  ok("ts: no arg shows epoch+iso", (await run("ts")).some((l) => l.includes("epoch ms")) && (await run("ts")).some((l) => l.includes("iso")));
  ok("ts: epoch s → iso", (await run("ts 1725000000")).join("").includes("2024-08-30"));
  ok("ts: epoch ms → iso", (await run("ts 1788998400000")).join("").includes("2026-09-10"));
  ok("ts: iso → epoch", (await run("ts 2026-09-10")).join("").includes("epoch ms"));
  ok("ts: garbage handled", (await run("ts notadate")).some((l) => l.includes("cannot parse")));
  const colA = await run("color #ff0080");
  ok("color: hex expands", colA.join("").includes("rgb(255, 0, 128)"));
  ok("color: hsl line", colA.some((l) => l.includes("hsl(330, 100%")));
  ok("color: contrast graded", colA.some((l) => l.includes("AA")));
  ok("color: forms agree", JSON.stringify(await run("color rgb 255 0 128")) === JSON.stringify(colA.slice(0, 5).map((l) => l)) || (await run("color rgb 255 0 128")).join("") === colA.join(""));
  ok("color: shorthand f00", (await run("color f00")).join("").includes("#ff0000"));
  ctx.fs.writeFile("/tmp/people.csv", "name,age,city\nmaya,25,tokyo\nraj,30,\"new delhi\"\n");
  const csvT = await run("csv /tmp/people.csv");
  ok("csv: aligned table", csvT.some((l) => l.includes("maya")) && csvT.some((l) => l.includes("2 data row(s), 3 column(s)")));
  ok("csv: quoted field intact", csvT.join(" ").includes("new delhi"));
  const csvJ = await run("csv json /tmp/people.csv");
  ok("csv: json objects", csvJ.some((l) => l.includes('{"name":"maya","age":"25","city":"tokyo"}')));
  ok("csv: missing file", (await run("csv /nope.csv")).some((l) => l.includes("no such file")));
  const cronA = await run("cron */15 9-17 * * 1-5");
  ok("cron: step expanded", cronA.some((l) => l.includes("0 15 30 45")));
  ok("cron: weekday names", cronA.some((l) => l.includes("mon tue wed thu fri")));
  ok("cron: next runs listed", cronA.filter((l) => l.includes("UTC")).length >= 1);
  ok("cron: invalid field refused", (await run("cron 99 * * * *")).some((l) => l.includes("invalid minute")));
  ok("cron: arity enforced", (await run("cron 1 2 3")).some((l) => l.includes("exactly 5 fields")));
  ok("help: dev section present", (await run("help")).some((l) => l.includes("DEV — developer tools")));
